/**
 * ♠ 台灣大老二 (Taiwan Big Two Page)
 * Features 2 Match Modes, Industry Standard Penalty Settlement, & Smart AI
 */
...
export async function renderBigTwo(container, params = {}) {
  ...
  container.innerHTML = `
    <div class="bigtwo-page animate-fade-in">
      ...
      <div class="topbar">
        <div class="topbar-left">
          ...
          <div class="topbar-title">
            ...
            <span class="badge badge-warning">業界標準結算</span>
          </div>
        </div>
        <div class="topbar-actions" style="display:flex;gap:8px;">
          <select id="select-bt-mode" style="padding:6px 12px;border-radius:12px;background:#1e293b;border:1px solid #38bdf8;color:#fff;font-weight:700;">
      ...
      <!-- ♠ Casino Navy Blue Table -->
      <div class="bigtwo-table">
        <!-- Top AI Player (P3) -->
        <div style="grid-row:1;grid-column:2;display:flex;flex-direction:column;align-items:center;">
          <div style="font-size:0.85rem;color:#38bdf8;font-weight:700;">🀃 北極吉伊 (<span id="p3-card-count">13</span>張)</div>
        ...
        <div style="grid-row:2;grid-column:1;display:flex;flex-direction:column;align-items:center;justify-content:center;">
          <div style="font-size:0.85rem;color:#38bdf8;font-weight:700;transform:rotate(-90deg);">🀂 西城飛鼠 (<span id="p4-card-count">13</span>張)</div>
        </div>

        <!-- Right AI Player (P2) -->
        <div style="grid-row:2;grid-column:3;display:flex;flex-direction:column;align-items:center;justify-content:center;">
          <div style="font-size:0.85rem;color:#38bdf8;font-weight:700;transform:rotate(90deg);">🀁 東區皮卡 (<span id="p2-card-count">13</span>張)</div>
        </div>

        <!-- Center Discard & Round Information -->
        <div class="bigtwo-center-area">
          <div style="font-size:0.85rem;color:#a7f3d0;font-weight:700;margin-bottom:8px;" id="bt-current-turn-label">
            當前回合: 玩家 (你)
          </div>
        ...
        <div class="bigtwo-player-hand">
          <div class="bigtwo-actions-bar">
            <button class="btn-bt-action btn-bt-play" id="btn-bt-play">♠ 出牌</button>
            <button class="btn-bt-action btn-bt-pass" id="btn-bt-pass">❌ PASS 過牌</button>
          </div>

          <div class="cards-hand-row" id="p1-hand-cards"></div>
          <div style="font-size:0.85rem;color:#facc15;font-weight:700;">玩家手牌 — 點擊牌卡選取或取消</div>
        </div>
      </div>
      ...
      <div id="bt-settlement-modal" style="display:none;position:fixed;inset:0;background:rgba(15,23,42,0.85);backdrop-filter:blur(10px);z-index:999;align-items:center;justify-content:center;">
        <div style="background:#1e293b;border:2px solid #f59e0b;border-radius:24px;padding:24px;width:90%;max-width:440px;color:#fff;text-align:center;box-shadow:0 16px 40px rgba(0,0,0,0.6);">
          <h2 style="font-size:1.6rem;color:#f59e0b;margin-bottom:12px;" id="settle-title">👑 遊戲局終結算</h2>
          <div style="font-size:1.1rem;font-weight:700;color:#38bdf8;margin-bottom:12px;" id="settle-winner-name">獲勝者: 玩家</div>
          
          <div id="settle-details-box" style="display:flex;flex-direction:column;gap:8px;background:rgba(15,23,42,0.6);padding:14px;border-radius:16px;text-align:left;font-size:0.9rem;margin-bottom:16px;"></div>

          <button class="btn btn-primary btn-block" id="btn-settle-restart">
  ...
  const renderUI = () => {
    ...
    if (lastBox) {
      if (state.lastCombo) {
        ...
        lastBox.innerHTML = `<span style="font-size:0.85rem;color:#a7f3d0;">自由出牌輪 (自由選擇任意合法牌型)</span>`;
      }
    }
