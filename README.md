...
> **Carrot Games** 是一個不需要任何伺服器後端、打開瀏覽器即可開始的純前端對戰遊戲平台。包含 **中國象棋 (Xiangqi)**、**俄羅斯方塊即時對戰 (Tetris Battle 2P)**、**德州撲克 (Texas Hold'em Poker)** 以及全新的 **魔法對戰 3D (Magic Fighter 3D)**。支援高智能 WebAssembly / Minimax AI 對戰與基於 WebRTC 的 P2P 點對點連線對弈，全站採用 **Happy Hues 粉嫩清爽主題** 與 **三麗鷗/吉卜力 Q 版繪畫風格**，並提供 **PWA 全自動零感更新** 與 **單手搖桿觸控** 支援。
...
### 4. 魔法對戰 3D (Magic Fighter 3D)
- **Three.js WebGL 3D 魔法空戰引擎**：重塑經典 NES《坦克大戰 (Battle City)》，防守地圖底部蘿蔔 HQ 水晶總部，擊退 5 波次攻勢敵軍。
- **360° 動態虛擬搖桿**：手機手勢模擬極速飛行。
- **經典 6 大寶箱道具**：無敵頭盔 (Helmet)、時鐘定身 (Clock)、手榴彈全滅 (Grenade)、星星火力升級 (Star 破鋼貫穿彈)、鐵鏟總部加固 (Shovel 鋼牆防線 15 秒)、戰機加命 (Extra Life)。
...
- **Three.js WebGL 3D 引擎**：`fighter-renderer-3d.js` 實現高對比度亮色光影與立體地形障礙。
...
### 4. 魔法對戰 3D (Magic Fighter 3D / Battle City Classic Rulebook)
- **遊戲目標**：重塑經典 NES《坦克大戰》，防守地圖底部 **蘿蔔 HQ 水晶總部**，擊退一共 5 波次攻勢敵軍！
- **6 大經典寶物道具**：
  - **無敵頭盔 (Helmet)**：8 秒無敵防禦光罩。
  - **時鐘定身 (Clock)**：凍結全場敵機 6 秒無法移動射擊。
  - **手榴彈 (Grenade)**：全滅爆破當前全場敵機。
  - **星星火力 (Star)**：升級雙發彈與破鋼貫穿子彈（可打碎鋼牆）。
  - **鐵鏟加固 (Shovel)**：將總部四周磚牆瞬間變更為堅硬鋼鐵牆 15 秒。
  - **戰機加命 (Extra Life)**：生命值 HP +1。
